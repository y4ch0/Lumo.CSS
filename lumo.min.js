...
if(!target.closest("details:has(menu)[open]")){document.querySelectorAll("details:has(menu)[open]").forEach((d)=>d.removeAttribute("open"))}
...
function initLumoScan(){document.querySelectorAll("article.tabs:not([data-lumo-init])").forEach((el)=>{el.setAttribute("data-lumo-init","true");const tabs=el.querySelectorAll('[role="tab"]');const panels=el.querySelectorAll('[role="tabpanel"]');tabs.forEach((tab,i)=>{const panel=panels[i];if(!tab.id){tab.id=`tab-${Math.random().toString(36).substr(2, 9)}`}
