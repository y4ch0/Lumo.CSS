...
function initLumo() {
    ...
    if (window._lumoGlobalListenersAttached) return initLumoScan();
    window._lumoGlobalListenersAttached = true;
...
// --- Re-runnable: scans for new Tabs, Carousels, Dropdowns (safe to call after React updates) ---
function initLumoScan() {
    // --- 3. Tabs Implementation ---
    document.querySelectorAll("article.tabs:not([data-lumo-init])").forEach((el) => {
        el.setAttribute("data-lumo-init", "true");
    ...
    document.querySelectorAll('[aria-roledescription="carousel"]:not([data-lumo-init])').forEach((carousel) => {
        carousel.setAttribute("data-lumo-init", "true");
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initLumo);
} else {
    initLumo();
}

window.initLumo = initLumo;
window.initLumoScan = initLumoScan;
