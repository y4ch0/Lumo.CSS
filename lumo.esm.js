...
export class Modal {
    ...
    _animate(direction) {
        return new Promise((resolve) => {
            ...
            const opacity = new Animation(new KeyframeEffect(this.el, [{ opacity: isOpening ? 0 : 1 }, { opacity: isOpening ? 1 : 0 }], { duration: MODAL_DURATION, easing: "ease" }), document.timeline);
            const move = new Animation(
                new KeyframeEffect(this.body, [{ transform: `translateY(${isOpening ? "-3rem" : "0"})` }, { transform: `translateY(${isOpening ? "0" : "-3rem"})` }], { duration: MODAL_DURATION, easing: "ease" }),
                document.timeline,
            );

            opacity.onfinish = resolve;
            opacity.play();
            move.play();
        });
    }
...
export class Carousel {
    ...
    _initIndicators() {
        this.indicatorsList = this.carousel.querySelector('[role="tablist"]');
        if (!this.indicatorsList) {
            ...
            this.carousel.appendChild(this.indicatorsList);
        }
...
// --- 3. GLOBAL INITIALIZER (The "Bootstrap" way) ---
export function initLumo() {
    // Modal & Dropdown delegation
    document.addEventListener("click", (e) => {
        ...
        if (openModalEl && (closeTrigger || (target === openModalEl && !openModalEl.hasAttribute("data-locked")))) {
    ...
    document.querySelectorAll('[aria-roledescription="carousel"]').forEach((c) => {
